...
import { Box, ArrowRight, Clock, Loader2, AlertCircle, RefreshCcw, Sparkles, BrainCircuit } from "lucide-react";
...
export default function DashboardPage() {
  ...
  if (authLoading) return (
    <div className="min-h-screen bg-[#04060f] flex flex-col items-center justify-center gap-6">
       ...
       <div className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500 animate-pulse">{t("Initializing Neural Link...")}</div>
    </div>
  );
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-8 py-12 relative z-10 space-y-16">
        <div className="flex items-center justify-between border-b border-white/5 pb-8">
          <div className="flex items-center gap-4">
            ...
            <div className="text-sm font-black tracking-[0.2em] text-white/40 uppercase">{t("Intelligence Array")}</div>
          </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-12">
              ...
              <AnimatePresence mode="wait">
                {isLoading ? (
                  <motion.div key="loading" className="flex flex-col items-center justify-center p-20 gap-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    ...
                    <div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 animate-pulse">{t("Syncing...")}</div>
                  </motion.div>
                ) : error ? (
                  <motion.div key="error" className="bg-red-500/5 border border-red-500/20 rounded-3xl p-12 text-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                    ...
                    <h3 className="text-xl font-bold text-white mb-2">{t("Sync Severed")}</h3>
                    <p className="text-slate-400 mb-6">{error}</p>
                    <Button variant="outline" onClick={fetchDashboardData} className="rounded-2xl border-white/10 bg-white/5 hover:bg-white/10"><RefreshCcw className="h-4 w-4 mr-2" /> {t("Reboot")}</Button>
                  </motion.div>
                ) : products.length === 0 ? (
                  <motion.div key="empty" className="bg-white/[0.02] border border-white/[0.05] border-dashed rounded-[2rem] p-16 text-center space-y-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                     <h3 className="text-2xl font-bold text-white">{t("Void is Empty")}</h3>
                     <p className="text-slate-400">{t("Initialize a product to track interactions.")}</p>
                  </motion.div>
                ) : (
                            ...
                            <CardContent className="space-y-4">
                             ...
                             <CardFooter className="flex gap-2">
                              <Link href={`/toolkit/${p._id}`} className="flex-1">
                                <Button className="w-full bg-white/5 hover:bg-white/10 text-white rounded-xl text-xs font-bold px-2">{t("VIEW MTRX ")}<ArrowRight className="h-4 w-4 ml-1" /></Button>
                              </Link>
                              <Link href={`/analytics/product/${p._id}`} className="flex-1">
                                <Button className="w-full bg-primary/10 hover:bg-primary/20 text-primary border border-primary/20 rounded-xl text-xs font-bold px-2">{t("ANALYSIS")}</Button>
                              </Link>
                            </CardFooter>
                          </Card>
                       </motion.div>
          ...
          <div className="xl:col-span-4 space-y-6">
            <h2 className="text-[12px] font-black uppercase tracking-[0.2em] text-slate-500 flex items-center gap-2 mb-8">
              <Sparkles className="h-4 w-4 text-blue-400" /> {t("Recent Telemetry")}
            </h2>
            <div className="bg-white/[0.01] border border-white/[0.03] rounded-[2rem] p-6 space-y-6">
              {reports.length === 0 && <p className="text-slate-500 text-sm">{t("No telemetry recorded yet.")}</p>}
              {reports.map((r) => (
                <div key={r._id} className="flex gap-4 p-4 rounded-2xl hover:bg-white/[0.02]">
                  <div className="h-10 w-10 rounded-full bg-white/5 flex items-center justify-center text-primary font-bold">{t("R")}</div>
                  <div>
                    <div className="text-sm font-bold text-white mb-1">{t("Session ")}{r._id.slice(-5)}</div>
                    <div className="text-xs text-slate-400 mb-2">{t("Score: ")}{r.overallScore?.toFixed(1) || '0.0'}</div>
